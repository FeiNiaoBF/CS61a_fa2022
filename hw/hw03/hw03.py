...
def pingpong(n):
    ...
    if n == 1:
        ...
        if helper_pingpong(n) > 0:
            return pingpong(n - 1) + 1
        else:
            return pingpong(n - 1) - 1

def helper_pingpong(n):
    if n <= 8:
        return 1
    else:
...
def count_coins(change):
    ...
    True
    """
    def helper_count_coins(change, coin):
        if change == 0:
            return 1
        elif change < 0 or not coin :
            return 0
        else:
            return helper_count_coins(change - coin, coin) + helper_count_coins(change, next_smaller_coin(coin))
    return helper_count_coins(change, 25) # maxcoin
